...
export default function VisionBoard({ lockInSummary }) {
  ...
  const todayTask = phase?.pillars?.flatMap(p => p.weeklyActions || []).find(Boolean) || 'Complete 1 action from your current phase'
        ...
        <div style={{
          ...
          <div style={{ textAlign: 'right' }}>
            <p style={{ fontSize: '0.76rem', color: 'rgba(255,255,255,0.8)', fontStyle: 'italic' }}>
              Protect it in Lock In
            </p>
            <p style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.72)', marginTop: '0.2rem' }}>
...
/* ── Pillar Card ── */
function PillarCard({ pl, editing, checked, phaseId, presetOpen, onCollapse, onUpdate, onUpdateArr, onAddArr, onDelArr, onCheck, onUpload, onDel, onPreset, onApplyPreset }) {
  return (
