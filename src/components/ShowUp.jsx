...
.showup-subroom-create{
  min-height:46px;
  border-radius:14px;
  border:1px dashed rgba(249,95,133,0.32);
  background:#fff;
  color:#f95f85;
  padding:0 28px;
  font-size:15px;
...
.showup-feed-card.is-activity{
  ...
  border:1px solid rgba(249,95,133,0.10);
}
.showup-feed-card.is-checkin{
  background:linear-gradient(135deg,#fff,#fff4f8);
  border:1px solid rgba(249,95,133,0.22);
  box-shadow:0 2px 12px rgba(249,95,133,0.08), 0 8px 28px rgba(77,49,66,0.05);
}
.showup-pulse-label{
  margin:0 0 8px;
  font-size:10px;
...
.showup-reaction-chip:hover{background:#fff0f4;color:#f95f85}
...
.showup-compact-modal{
  width:min(420px, calc(100vw - 32px));
  border-radius:18px;
  padding:18px;
  background:#fff;
  box-shadow:0 24px 80px rgba(77,49,66,0.22);
}
.showup-toggle-row{
  display:flex;
  align-items:center;
...
.showup-toggle-pill button.is-active{
  background:#fff;
  color:#f95f85;
  box-shadow:0 6px 18px rgba(77,49,66,0.10);
}
...
export default function ShowUp({ user, profileData: externalProfileData, onGoToDailyStreaks }) {
  ...
  useEffect(() => {
    ...
    async function bootstrap() {
      ...
      try {
        ...
        const remoteJoinedRoom = await loadJoinedRoom(nextProfile)
        if (!alive) return
        if (remoteJoinedRoom && !selectedRoom) {
          setSelectedRoom(remoteJoinedRoom)
          setActiveTab('live')
          writePresence(remoteJoinedRoom, nextProfile.id, 'active')
          setPresenceTick(current => current + 1)
        }
  ...
  async function loadJoinedRoom(nextProfile = profile) {
    if (!supabase || !nextProfile?.id) {
      setJoinedRoomName('')
      return ''
    }
    ...
    try {
      const { data, error: joinedError } = await supabase
        .from('show_up_checkins')
        .select('room_name,created_at,check_in_time')
      ...
      console.error('Show Up joined room failed', nextError)
      setJoinedRoomName('')
      return ''
    }
  }
    ...
    try {
      ...
      const remotePosts = (data || []).map((post, index) => {
        ...
        return {
          ...
          authorAvatarUrl: post.author_avatar_url || memberMatch?.avatar_url || memberMatch?.avatar || (authorId === profile.id ? profile.avatar : '') || '',
          authorBio: post.author_bio || memberMatch?.about || memberMatch?.bio || '',
          anonymous: false,
          ...
          pulseLabel: remoteWelcome
            ...
            : remoteRecap
              ...
              : '',
          targetUserId: remoteTargetUserId || '',
          postStyle: remoteCheckin ? 'checkin' : (remoteRecap ? 'recap' : (remotePulseFormat ? 'pulse' : '')),
  ...
  async function handleCheckIn() {
    ...
    const patch = {
      ...
      created_at: currentMember?.created_at || new Date().toISOString(),
    }
    setCheckedIn(true)
    setTaskDone(false)
    setLocalSessionMember(patch)
    setDoneTime('')
    ...
    try {
      ...
      await createFeedPost({
        text: profile.about || `${profile.name} checked in for today's challenge.`,
        image: '',
        anonymous: false,
        postStyle: 'checkin',
        author: {
  ...
  async function handleMarkDone() {
    ...
    setDoneBusy(true)
    setTaskDone(true)
    setLocalSessionMember(current => ({ ...(current || donePatch), ...donePatch }))
    setDoneTime(nowIso)
    setToast('')
    setStoredActiveRoom('')
  ...
  }, [checkedIn, currentMember, localSessionMember, profile, selectedRoom, taskDone])
  ...
  async function handleCreateSubRoom() {
    ...
    if (supabase) {
      ...
      if (error) {
        console.error('Show Up sub-room create failed', error)
        setError('Could not create sub-room in Supabase.')
        return
      }
      if (data) setBottomToast('Sub-room created.')
    } else {
                  ...
                  <div className="showup-feed-header">
                    <div className="showup-feed-author">
                      <button
                        type="button"
                        className="showup-avatar"
                        style={{ overflow: 'hidden', padding: 0, flexShrink: 0, cursor: (!post.anonymous && post.authorAvatarUrl) ? 'pointer' : 'default', border: 'none', background: 'transparent' }}
                        onClick={event => {
                          if (!post.anonymous && post.authorAvatarUrl) {
                            event.stopPropagation()
                            setLightboxMedia({ url: post.authorAvatarUrl, kind: 'image' })
                          }
                        }}
