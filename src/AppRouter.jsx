...
import DailyStreaksPage from './pages/features/DailyStreaksPage'
import JournalFeaturePage from './pages/features/JournalPage'
...
function getAuthReturnValue() {
  ...
  try { return localStorage.getItem(AUTH_RETURN_KEY) } catch { return null }
}
...
function hasRemoteOnboardingComplete(user) {
  ...
  return Boolean(
    metadata.onboarding_completed || metadata.phasr_onboarded || metadata.product_ready
  )
}
...
export default function AppRouter() {
  ...
  const [theme, setTheme] = useState(() => {
    const saved = localStorage.getItem('phasr_theme')
  ...
  useEffect(() => {
    document.title = 'PHASR — Monthly Goals & Accountability for Women'
  ...
  useEffect(() => {
    ...
    if (identifier) safeLocalSet(ACTIVE_USER_KEY, identifier)
    cacheUser(user)
  ...
  useEffect(() => {
    if (user && location.pathname === '/dashboard' && !onboarded) {
      localStorage.setItem(ONBOARDING_ACTIVE_KEY, 'true')
      return
    }
    if (user && onboarded) localStorage.removeItem(ONBOARDING_ACTIVE_KEY)
  }, [user, location.pathname, onboarded])
  ...
  useEffect(() => {
    ...
    const { data: listener } = supabase.auth.onAuthStateChange((event, session) => {
      ...
      setUser(resolvedUser)
      setOnboarded(getStoredOnboarded(resolvedUser) || getStoredOnboarded(nextUser))
  ...
  function handleAuthSuccess(nextUser) {
    setUser(nextUser)
    const isOnboarded = getStoredOnboarded(nextUser)
  ...
  return (
    <>
      ...
      <Routes>
        ...
        <Route path="/features/vision-boards" element={<VisionBoardsPage />} />
        <Route path="/features/daily-streaks" element={<DailyStreaksPage />} />
